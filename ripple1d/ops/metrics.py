...
from ripple1d.consts import HYDROFABRIC_CRS, METERS_PER_FOOT, NWM_ID_COL, NWM_TO_ID_COL
...
class ConflationMetrics:
    ...
    def overlapped_reaches(self, to_reaches: gpd.GeoDataFrame) -> dict:
        ...
        for i, row in to_reaches.iterrows():
            if row[geom_name].intersects(self.xs_gdf.union_all()):
                ...
                reach_ids.append({"id": str(row[NWM_ID_COL]), "overlap": int(overlap)})
        return reach_ids
    ...
    def eclipsed_reaches(self, network_reaches: gpd.GeoDataFrame) -> dict:
        ...
        return [str(row[NWM_ID_COL]) for _, row in eclipsed_reaches.iterrows()]
    ...
    * **coverage.** These metrics quantify the portion of the NWM reach between
    ...
    for network_id in conflation_parameters["reaches"].keys():
        try:
            ...
            network_reach = linemerge(network_reaches.loc[network_reaches[NWM_ID_COL] == int(network_id)].geometry.iloc[0])
            ...
            overlapped_reaches = cm.overlapped_reaches(
                network_reaches[network_reaches[NWM_ID_COL].isin([int(to_id), next_to_id])]
            )
...
def combine_reaches(network_reaches: gpd.GeoDataFrame, network_id: str) -> LineString:
    ...
    reach = network_reaches.loc[network_reaches[NWM_ID_COL] == int(network_id), :]
    to_reach = network_reaches.loc[network_reaches[NWM_ID_COL] == int(reach[NWM_TO_ID_COL].iloc[0]), :]

    if to_reach.empty:
        return linemerge(reach.geometry.iloc[0])
    else:
        next_to_reach = network_reaches.loc[network_reaches[NWM_ID_COL] == int(to_reach[NWM_TO_ID_COL].iloc[0]), :]
