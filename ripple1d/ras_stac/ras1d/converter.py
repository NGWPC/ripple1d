...
from ripple1d.ras_stac.ras1d.utils.classes import (
    GenericAsset,
    GeometryAsset,
    GeopackageAsset,
    NullGeometryAsset,
...
from ripple1d.ras_stac.ras1d.utils.common import (
...
from ripple1d.ras_stac.ras1d.utils.s3_utils import (
    gather_dir_s3,
    save_bytes_s3,
    save_file_s3,
    str_from_s3,
)
from ripple1d.ras_stac.ras1d.utils.stac_utils import generate_asset
...
class Converter:
    ...
    def export_gpkg(self, out_path: str, user_metadata: dict) -> None:
        ...
        meta |= user_metadata
        create_non_spatial_table(tmp_out_path, meta)
        if file_location(out_path) != "local":
...
def gpkg_from_ras(source_model_directory: str, crs: str | None, metadata: dict):
    """Convert a HEC-RAS model to a STAC item and save to same directory."""
    logging.info(f"Processing model with crs {crs} at prefix {source_model_directory}")
    logging.info("Discovering model contents")
    # Data prep
    if file_location(source_model_directory) == "local":
        source_model_directory = source_model_directory.rstrip("/\\") + os.sep
    else:
        source_model_directory = source_model_directory.rstrip("/") + "/"
    crs = CRS(crs).to_wkt()

    # Processing
    converter = from_directory(source_model_directory, crs)
    converter.check_for_mip()

    # Define paths
    thumb_path = source_model_directory + "Thumbnail.png"
    gpkg_path = source_model_directory + f"{converter.idx}.gpkg"
    stac_path = source_model_directory + f"{converter.idx}.json"
    ...
    if converter.crs is not None:
        ...
        converter.export_gpkg(gpkg_path, metadata)

    logging.info(f"Generating STAC item at {stac_path}")
    converter.export_stac(stac_path)
    return {"in_path": source_model_directory, "crs": crs, "thumb_path": thumb_path, "stac_path": stac_path}
