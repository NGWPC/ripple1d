...
from ripple1d.ras_stac.ras1d.data.us_geom import us_bounds
from ripple1d.ras_stac.ras1d.utils.common import file_location
from ripple1d.ras_stac.ras1d.utils.ras_utils import (
    check_xs_direction,
...
from ripple1d.ras_stac.ras1d.utils.s3_utils import key_metadata, split_s3_key, str_from_s3
...
class ProjectAsset(GenericAsset):
    ...
    @cache_data
    def units(self) -> str:
        """Units of the HEC-RAS project (English or Metric)."""
...
class SteadyFlowAsset(GenericAsset):
    ...
    def _extra_fields(self):
        ...
        ex["profile_names"] = self.profile_names
        return ex
...
class UnsteadyFlowAsset(GenericAsset):
    ...
    @cache_data
    def profile_names(self):
        results = search_contents(self.file_str.splitlines(), "Profile Names", expect_one=False)
        if not results:
            return []
